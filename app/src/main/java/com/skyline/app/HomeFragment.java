...
public class HomeFragment extends Fragment {
        ...
        com.skyline.app.network.RetrofitClient.getInstance().getPromotions().enqueue(new retrofit2.Callback<List<Promotion>>() {
            ...
            public void onResponse(retrofit2.Call<List<Promotion>> call, retrofit2.Response<List<Promotion>> response) {
                ...
                if (response.isSuccessful() && response.body() != null && !response.body().isEmpty()) {
                    ...
                    // Lọc 4 khuyến mãi từ 4 danh mục khác nhau
                    List<Promotion> filteredList = new ArrayList<>();
                    String[] targetCategories = {"EXCLUSIVE", "FIXED", "NEW_USER", "PAYMENT"};
                    
                    for (String cat : targetCategories) {
                        for (Promotion p : allPromotions) {
                            if (cat.equalsIgnoreCase(p.getCategory())) {
                                filteredList.add(p);
                                break; // Lấy 1 cái mỗi loại rồi chuyển qua category tiếp theo
                            }
                        }
                    }

                    // Nếu không đủ 4 cái theo category, lấy thêm cho đủ (hoặc dùng list gốc nếu trống)
                    if (filteredList.isEmpty()) filteredList = allPromotions.size() > 4 ? allPromotions.subList(0, 4) : allPromotions;
                    
                    binding.promoPager.setAdapter(new PromotionAdapter(filteredList, item -> {
                        // Khi nhấn "Tìm hiểu thêm" trong Pager, mở chi tiết của mã đó
                        Intent intent = new Intent(requireContext(), PromotionsActivity.class);
                        intent.putExtra("OPEN_PROMO_ID", item.getId());
                        startActivity(intent);
                    }));
                    createDots(binding.promoDots, filteredList.size());
    ...
    private void setupClicks() {
        ...
        // Nhấn "Khám phá ngay", Ảnh banner hoặc tiêu đề đều mở chi tiết mã "Thứ 6 Mở App"
        View.OnClickListener promoListener = v -> {
            Intent intent = new Intent(requireContext(), PromotionsActivity.class);
            // Link trực tiếp đến ID của mã "Thứ 6 Mở App" trong MongoDB
            intent.putExtra("OPEN_PROMO_ID", "6a4fc9d6457caed1dce3e5b8");
            startActivity(intent);
        };

        binding.btnExploreNow.setOnClickListener(promoListener);
        binding.imgBrandBanner.setOnClickListener(promoListener);
        binding.tvDealTitle.setOnClickListener(promoListener);
